$(function () {
    ...
    function updateStatus(trabajoId, nuevoStatus) {
        $.ajax({
            url: `/erp/taller/update_status/${trabajoId}/`,
            type: "POST",
    ...
    $('#data').DataTable({
        ...
        drawCallback: function () {
            $(".status-select").select2({
                width: '100%',
                templateResult: formatStatus,    // Personaliza las opciones
                templateSelection: formatStatus  // Personaliza la selección
            });
        }
    });
    ...
    function formatStatus(state) {
        ...
        let colorMap = {
            "Sin Comenzar": "#f44336",           // Rojo
            "En el Taller": "#ff9800",           // Naranja
            "Entregado": "#4caf50",              // Verde
            "Listo y Sin Retirar": "#2196f3",    // Azul
            "Cotización": "#9c27b0"              // Morado
        };
        let color = colorMap[state.text] || "#000";  // Color negro si no se encuentra el estado
        return $(
            `<span style="color:${color}; font-weight:bold;">${state.text}</span>`
        );
    }
...
$(function () {
    ...
    $('#data').DataTable({
        ...
        drawCallback: function () {
            $(".status-select").select2({
                width: '100%',
                templateResult: formatStatusBadge,    // Personaliza opciones (lista desplegable)
                templateSelection: formatStatusBadge, // Personaliza opción seleccionada
                escapeMarkup: function (markup) { return markup; } // ⚡ Permite HTML en Select2
            });
        }
    });

    // 🎨 Asigna una etiqueta (badge) a cada estado
    function formatStatusBadge(state) {
        ...
        return `<span class="${badgeClass}" style="padding: 5px 10px; border-radius: 10px;">${state.text}</span>`;
    }
