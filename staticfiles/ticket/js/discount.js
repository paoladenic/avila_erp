...
var vents = {
    ...
    calculate_invoice: function () {
        ...
        $.each(this.items.products, function (pos, dict) {
            dict.pos = pos;
            dict.subtotal = vents.round(dict.cant * parseFloat(dict.pvp) * (1 - (discount / 100)));
            subtotal += parseFloat(dict.subtotal);
        });

        this.items.subtotal = this.round(subtotal);
        this.items.iva = this.round(subtotal * iva);
        this.items.total = this.round(subtotal + parseFloat(this.items.iva));
    ...
    update_pagado: function () {
        this.items.pagado = this.round(parseFloat(this.items.cash) + parseFloat(this.items.card));
...
$(function () {
    ...
    $('input[name="cash"]').on('change', function () {
        vents.items.cash = parseFloat($(this).val()).toFixed(2) || 0.00;
        vents.update_pagado();
    });

    $('input[name="card"]').on('change', function () {
        vents.items.card = parseFloat($(this).val()).toFixed(2) || 0.00;
        vents.update_pagado();
    });
    ...
    $('#frmTicket').on('submit', function (e) {
        ...
        vents.items.cash = parseFloat($('input[name="cash"]').val()).toFixed(2) || 0.00;
        vents.items.card = parseFloat($('input[name="card"]').val()).toFixed(2) || 0.00;
        vents.items.pagado = (parseFloat(vents.items.cash) + parseFloat(vents.items.card)).toFixed(2);

        if (vents.items.pagado !== parseFloat(vents.items.total).toFixed(2)) {
